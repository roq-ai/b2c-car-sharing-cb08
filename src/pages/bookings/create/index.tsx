...
function BookingCreatePage() {
  ...
  const formik = useFormik<BookingInterface>({
    initialValues: {
      ...
      drop_off_time: new Date(new Date().toDateString()),
    ...
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          <FormControl id="drop_off_time" mb="4">
            <FormLabel>Drop Off Time</FormLabel>
            <Box display="flex" maxWidth="100px" alignItems="center">
              <DatePicker
                dateFormat={'dd/MM/yyyy'}
                selected={formik.values?.drop_off_time ? new Date(formik.values?.drop_off_time) : null}
                onChange={(value: Date) => formik.setFieldValue('drop_off_time', value)}
              />
              <Box zIndex={2}>
                <FiEdit3 />
              </Box>
